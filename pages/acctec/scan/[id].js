...
import TimeTable from "@components/acctec/TimeTable";

const GroupCard = ({ group, selected, selectable }) => {
  const [hover, setHover] = React.useState(false);
  let background = (selected || hover) && selectable ? "skyblue" : "lightgray";
...
export const ScanPK = () => {
  ...
  }, [selectedGroup, selectedGroupIndexes]);
  ...
  function putToken() {
    ...
    if (selectedGroupIndexes?.length > 0) {
      ...
      selectedGroupIndexes.forEach((i) => {
        if (i !== -1) {
          // No idea why -1 appears as an index but it does.
      ...
      {!tagExists && (
        <Container fixed>
          ...
          {selectedGroupIndexes?.length > 0 && (
            <RoundBox sx={{ p: 2 }}>
              <TimeTable onChange={(state) => {}} segments={[]} />
            </RoundBox>
          )}
          {selectedGroupIndexes?.length > 0 && (
            <RoundBox sx={{ p: 2 }}>
